...
本目录的说明文件纳入 Git；`local/` 存放完整本地数据包，由 `.gitignore` 整目录排除。标注协议中的示例均为原创演示；自动生成的候选不等于经人评确认的 gold。

2026-09-12 已按用户指定位置将六套公开数据、agentic 小样本和六组真实来源改写候选保存到桌面；2026-09-15 已迁入本目录的 `local/`，未导入 Git。见[实际采集记录](../09-12-local-data-and-rewrite-pilot.zh-CN.md)。这些候选尚未经过独立人评。
...
全库统一的正文层、审计与最小文本导出见[正文清理说明](../09-12-text-cleaning.zh-CN.md)。数据位于 `~/Desktop/plainvoice/data/local/`。清理导出保留每个原任务的角色、null／空字符串及多参考区别，不自动产生人类偏好标签。
