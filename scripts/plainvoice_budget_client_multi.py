...
API:
    client = BudgetClient(api_key=key, ledger_path=path, budget_usd='1', max_calls=12)
...
The $1 hard ceiling covers inference, not taxes or credit-purchase fees.
"""
...
PROFILES = {
    'google/gemini-2.5-flash-lite': {
        'canonical': 'google/gemini-2.5-flash-lite',
        'input_per_million': Decimal('0.10'), 'output_per_million': Decimal('0.40'),
        'providers': ('google-vertex', 'google-vertex/eu'),
        'label': 'Gemini 2.5 Flash Lite'},
    'google/gemini-3.1-flash-lite': {
        'canonical': 'google/gemini-3.1-flash-lite-20260507',
        'input_per_million': Decimal('0.25'), 'output_per_million': Decimal('1.50'),
        'providers': ('google-vertex/global',),
...
class BudgetClient:
    def __init__(self, *, api_key, ledger_path, budget_usd='1', max_calls=12,
                 timeout=90, transport=None, model=MODEL):
        if model not in PROFILES:
            raise BudgetError('Unsupported comparison model profile.')
        self.model = model
        self.profile = copy.deepcopy(PROFILES[model])
        self.input_rate = self.profile['input_per_million'] / 1_000_000
        self.output_rate = self.profile['output_per_million'] / 1_000_000
        self.response_models = {self.model, self.profile['canonical']}
        ...
        if not 0 < budget <= MAX_BUDGET or not 0 < max_calls <= 10000:
            raise BudgetError('Budget must be positive and at most $1; call limit must be 1–10000.')
        ...
        with self._state.lock:
            if not self.path.exists():
                self._save({'version': 1, 'model': self.model, 'budget_usd': str(budget),
    ...
    def __repr__(self):
        return '<BudgetClient model=' + self.model + ' key=REDACTED>'

    def _read(self):
        try:
            d = json.loads(self.path.read_text('utf-8'))
            if (d['version'] != 1 or d['model'] != self.model or not isinstance(d['halted'], bool)
    ...
    def summary(self):
        with self._state.lock:
            ...
            return {'model': self.model, 'budget_usd': str(self.budget), 'spent_usd': str(spent),
    ...
    def _fetch_preflight(self):
        ...
        try:
            ...
            matching = [m for m in catalog if m.get('id') == self.model]
            if len(matching) != 1:
                raise ValueError()
            model = matching[0]
            if model.get('canonical_slug') != self.profile['canonical'] or model.get('expiration_date') is not None:
                raise ValueError()
            ...
            if not params.issubset(model.get('supported_parameters', [])):
                raise ValueError()
            if (_decimal(model['pricing']['prompt']) != self.input_rate
                    or _decimal(model['pricing']['completion']) != self.output_rate
                    or model.get('reasoning', {}).get('mandatory') is True):
                raise ValueError()
            endpoints = self._get('/models/' + self.model + '/endpoints')['data']['endpoints']
            zdr = self._get('/endpoints/zdr')['data']
            zdr_tags = {e['tag'] for e in zdr if e.get('model_id') == self.model and e.get('status') == 0}
            eligible = []
            for e in endpoints:
                p = e['pricing']
                if (e.get('status') == 0 and e.get('tag') in zdr_tags
                        and e.get('tag') in self.profile['providers']
                        and params.issubset(e.get('supported_parameters', []))
                        and _decimal(p['prompt']) <= self.input_rate
                        and _decimal(p['completion']) <= self.output_rate
                        and _decimal(p.get('internal_reasoning', '0')) <= self.output_rate
                        and _decimal(p.get('request', '0')) == 0):
                    eligible.append(e)
            if not eligible:
                raise ValueError()
            return {'model': self.model, 'canonical_slug': model['canonical_slug'],
                    'input_usd_per_million': str(self.profile['input_per_million']),
                    'output_usd_per_million': str(self.profile['output_per_million']),
    ...
    def _reserve(self, input_bound, max_tokens, metadata):
        reservation = Decimal(input_bound) * self.input_rate + Decimal(max_tokens) * self.output_rate
        with self._state.lock:
            d = self._read()
    ...
    def chat(self, *, messages, max_tokens, json_schema=None, schema_name='plainvoice', temperature=0.7):
        ...
        body = {'model': self.model, 'messages': clean_messages, 'max_tokens': max_tokens,
                'temperature': temperature, 'stream': False,
                'reasoning': {'enabled': False},
                'provider': {'only': verified['provider_tags'], 'allow_fallbacks': False,
                             'require_parameters': True, 'data_collection': 'deny', 'zdr': True,
                             'max_price': {'prompt': float(self.profile['input_per_million']),
                                           'completion': float(self.profile['output_per_million'])}}}
        ...
        try:
            ...
            if not finish or not isinstance(choice.get('message'), dict):
                raise ValueError()
            metadata.update(model=self.model if model in self.response_models else 'unexpected_model',
                            served_model=model if model in self.response_models else 'unexpected_model',
