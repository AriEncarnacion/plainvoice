...
from plainvoice_budget_client_multi import BudgetClient, BudgetError, PROFILES

VERSION = 'plainvoice-enrichment-svo-v3'
DEFAULT_MODEL = 'google/gemini-2.5-flash-lite'

def profile_id(model):
    return model.split('/', 1)[1].replace('.', '')

def pair_id(q):
    return VERSION + ':' + profile_id(q.get('_model', DEFAULT_MODEL)) + ':' + digest(q['queue_id'])[:24]
...
def make_pair(q, output, extraction, packet, meta, qa, relative_dir):
    ...
    collection = 'ai-enrichment-comparison'
    model = q.get('_model', DEFAULT_MODEL)
    model_label = PROFILES[model]['label']
    title = original.get('title') or q['source_id']
    return {
        'id': pair_id(q),
        'collection': collection,
        'collection_title': 'AI 模型对比 · ' + model_label,
        ...
        'right': {'label': 'AI rewrite · ' + model_label + ' · 待人评',
...
def run(args):
    ...
    if not 1 <= len(rows) <= 6 or any('article' not in q['source_unit_type'] for q in rows):
        raise ValueError('comparison_requires_at_most_six_complete_articles')
    if args.workers < 1:
    ...
    config = {'version': VERSION, 'model': args.model,
    ...
    client = BudgetClient(api_key=key, ledger_path=args.output / 'budget-ledger.json',
                          budget_usd=args.budget, max_calls=args.max_calls, timeout=300, model=args.model)
    key = None
    ...
    todo = [q for q in rows if pair_id(q) not in done_ids]

    def one(q):
        item_dir = args.output / 'items' / (profile_id(args.model) + '-' + digest(q['queue_id'])[:24])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', choices=sorted(PROFILES), default=DEFAULT_MODEL)
